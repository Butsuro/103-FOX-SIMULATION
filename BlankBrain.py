...
def weighted_random_choice_cayote(a, b, c, d):
   # Define options and their dynamic weights
   options = ["Hunger", "Sleep Need", "Go to Friend", "Randomness"]
   weights = [a, b, c, d]  # These values change each time the function runs
...
def BrainCAYOTE(self, array):
   ...
   if sleep_timer > 0:
       sleep_timer = sleep_timer - (1/17280)
       return [0,0]
   if hunger >= 1 and exists("food"):
       if atathing("food", self):
           hunger = 0
   ...
   if FamilyTime > 0:
       ...
       option = weighted_random_choice_cayote(hunger, sleep_need,GoToFreind, randomness)
